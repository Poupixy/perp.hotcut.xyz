...
export const Route = createFileRoute("/api/nfts/collections/ingest")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          const { enqueueAllowedCollectionIngestion } = await import("@/jobs/ingestAllowedCollectionsJob");
          ...
          const result = enqueueAllowedCollectionIngestion({ collectionAddress: body.collectionAddress, limit: body.limit, maxPages: body.maxPages });
          return Response.json({ ...result, status: "queued" }, { headers: { "Cache-Control": "no-store" } });
        } catch (error) {
